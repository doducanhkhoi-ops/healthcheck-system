...
function Get-FolderSize ($path, $unit="MB") {
...
$memReductPaths = @(
    ...
    "C:\Program Files (x86)\Mem Reduct\memreduct.exe"
)
$memReductExe = $null
foreach ($mrPath in $memReductPaths) {
    if (Test-Path $mrPath) {
        $memReductExe = $mrPath
        break
    }
...
if ($memReductExe) {
    try {
        ...
        $usedRamBefore = [math]::Round(($osBefore.TotalVisibleMemorySize - $osBefore.FreePhysicalMemory) / 1024, 2)
        
        Start-Process -FilePath $memReductExe -ArgumentList "-clean" -WindowStyle Hidden
        Start-Sleep -Seconds 3
        
        $osAfter = Get-CimInstance Win32_OperatingSystem
        $usedRamAfter = [math]::Round(($osAfter.TotalVisibleMemorySize - $osAfter.FreePhysicalMemory) / 1024, 2)
        
        $ramFreed = [math]::Max(0, [math]::Round($usedRamBefore - $usedRamAfter, 2))
        
        $cleanLog += [PSCustomObject]@{
            Target = "Bộ nhớ RAM (Mem Reduct)"
            Status = "Đã dọn dẹp"
            Freed = "$ramFreed MB"
            Note = "Tự động kích hoạt Mem Reduct giải phóng bộ nhớ RAM"
        }
    } catch {
        $cleanLog += [PSCustomObject]@{
            Target = "Bộ nhớ RAM (Mem Reduct)"
            Status = "Lỗi"
            Freed = "0 MB"
            Note = "Lỗi trong quá trình gọi Mem Reduct"
        }
    }
} else {
    $cleanLog += [PSCustomObject]@{
        ...
        Status = "Bỏ qua"
        Freed = "0 MB"
        Note = "Hệ thống chưa cài đặt Mem Reduct. Vui lòng cài đặt để tối ưu RAM tự động."
    }
}
...
foreach ($ct in $cTargets) {
    if (Test-Path $ct.Path) {
        $szGB = Get-FolderSize -unit "GB" $ct.Path
        if ($szGB -gt 0.2) {
